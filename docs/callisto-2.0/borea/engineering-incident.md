...
## Phát hiện sự cố
Detection channels: Grafana alerts (metrics), Sentry (errors), PagerDuty (synthetic monitors), customer reports (#support Slack), social media monitoring. Auto-incident creation trên Opsgenie khi alert P1/P2.
...
## Quy trình xử lý
Bước 1: Auto-detect & create incident (0 min). Bước 2: IC join war room (3 min). Bước 3: Classify & assemble team (5 min). Bước 4: Execute runbook → investigate (parallel). Bước 5: Mitigate first, root cause later. Bước 6: Verify & monitor. Bước 7: Close.
...
## Postmortem
P1 postmortem trong 24 giờ, P2 trong 48 giờ. Blameless culture — focus on systems, not people. Format: 5-Whys + timeline + action items. Published trên Notion, accessible toàn engineering. Monthly postmortem review session.
