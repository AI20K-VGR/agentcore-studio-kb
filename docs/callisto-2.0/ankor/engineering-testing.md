...
## Chiến lược testing
Ankor áp dụng testing pyramid: nhiều unit test (70%), vừa phải integration test (20%), ít E2E test (10%).
Mọi code mới phải có test.

## Unit test
Unit test bắt buộc cho mọi business logic.
Coverage tối thiểu 80% cho code mới, 70% overall.
Framework: Jest (frontend), pytest (backend).
Mock external dependencies.

## Integration test
Integration test cho API endpoints, database queries, và service-to-service communication.
Sử dụng test containers (Docker) cho database và message queue.
Chạy trong CI, target dưới 10 phút.
...
## E2E test
E2E test cho critical user flows (login, checkout, payment).
Sử dụng Playwright.

## Test trước khi merge
CI pipeline chạy: lint → unit test → integration test cho mỗi PR.
PR không pass test không được merge.
...
## Security test
SAST (Snyk) chạy trong CI.
Dependency audit hằng tuần.
DAST (OWASP ZAP) chạy trên staging hằng tuần.
Security test coverage cho OWASP Top 10 vulnerabilities.
