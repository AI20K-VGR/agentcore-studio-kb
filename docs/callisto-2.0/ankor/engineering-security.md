...
## Quản lý mật khẩu
Mật khẩu tối thiểu 12 ký tự, bao gồm chữ hoa, chữ thường, số, và ký tự đặc biệt. Đổi mật khẩu mỗi 90 ngày. Sử dụng 1Password (công ty cấp) để quản lý mật khẩu. Không dùng lại mật khẩu.

## Xác thực đa yếu tố
MFA bắt buộc cho tất cả hệ thống: email, VPN, GitHub, cloud console, SAP. Hỗ trợ: authenticator app (ưu tiên), SMS (backup). Hardware key (YubiKey) cấp cho team Engineering và Finance.
...
## Bảo mật ứng dụng
OWASP Top 10 compliance bắt buộc. Security review cho mọi feature mới. SAST scan (Snyk) trong CI pipeline. Penetration testing 1 lần/năm bởi bên thứ ba. Bug bounty program (hạn mức: 500 USD – 5.000 USD).
...
Security incidents báo ngay cho Security team qua email security@ankor.vn hoặc Slack #security-alerts. Response time: Critical trong 1 giờ, High trong 4 giờ. Forensic investigation bởi Security team.
